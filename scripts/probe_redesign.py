...
ULSAN_SGG  = "31110 31140 31170 31200 31710"   # 중/남/동/북/울주
TIMEOUT = 20
...
def ecos_probe(stat_code, cycle="Q", start="2023", end="2024", item="?"):
    if not ECOS_KEY:
    ...
    try:
        ...
        return {"http":r.status_code,"ok":r.status_code==200,"raw":r.text[:200]}
    except Exception as e:
        return {"ok":False,"exc":str(e)[:160]}
...
PROBES = [
    ...
    # 사용자 제시 DT_1YL20711 → 오류. 정정 코드 DT_1YL20921 확인
    ("② 재정자립도",    "KOSIS DT_1YL20921(정정)", lambda: kosis_probe("DT_1YL20921")),
    # ✅ 가용 (단 시도단위 — 구군 분해 불가)
