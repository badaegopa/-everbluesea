...
KOSIS_TABLES = {
    "population": {"orgId":"101","tblId":"DT_1B040A3","itmId":"T20","objL1":"31","prdSe":"M","newEstPrdCnt":"1"},
...
def fetch_kosis(api_key, params):
    ...
    r.raise_for_status()
    data = r.json()
    sys.stderr.write(f"[debug] kosis type={type(data).__name__} len={len(data) if isinstance(data,list) else 'n/a'} sample={str(data)[:300]}\n")
    return data
...
def sgis_token(key, secret):
    ...
    r = requests.get(SGIS_AUTH, params={"consumer_key":key,"consumer_secret":secret},
...
def fetch_sgis_population(token):
    ...
    for name, sgg in ULSAN_SGG.items():
        r = requests.get(SGIS_STATS_POPULATION,
...
def main():
    ...
    try:
        ...
        sys.stderr.write("[ok] SGIS token acquired\n")
